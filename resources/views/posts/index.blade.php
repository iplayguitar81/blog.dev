    ...
    <div class="table">
        <table class="uk-table uk-table-hover uk-table-striped">
            ...
            @foreach($posts as $item)
                ...
                <tr>
                    {{--<td>{{ $x }}</td>--}}
                    <td><a href="{{ url('posts', $item->id) }}">{{ $item->title }}</a></td><td>{{ $item->body }}</td><td><img class="uk-thumbnail uk-thumbnail-mini uk-responsive-height" src="../images/{{ $item->imgPath}}"></td>
                    @can('isAdmin')
                    <td>
